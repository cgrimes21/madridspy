...
world
	OpenPort(t)
		CRASH("The game is meant for single player only.")
		return 0
*/
obj
	li
...
var/ed =  0	//world needs to finish first
...
world
	...
	//if fexists players/banned/address/src.client.address
		...
		wlog<<"[get_time()] World initializing."

		create_names()
		wlog<<"[get_time()] Created names, as follows: [list2params(gen_names)]"
		...
		wlog<<"[get_time()] year calculated; [rev]."
...
//		generate_networks()
		...
		spawn(20)	//give the world a time to finish the above procs
			wlog<<"[get_time()] World main loop initializing, now authorized for logins"
			ed = 1
			while(world)
				sleep(10)
				rankers = list()
